/**
 * Pure-JS HMAC-SHA256, used as a fallback when the Web Crypto API
 * (crypto.subtle) is unavailable — i.e. when the app is served over a
 * non-secure context (plain http:// on an IP/hostname, e.g. Docker).
 *
 * Web Crypto is only exposed in secure contexts (https:// or localhost),
 * so this keeps HS256 JWT signing working everywhere.
 */
...
function sha256(bytes: Uint8Array): Uint8Array {
  ...
  msg.set(bytes)
  msg[l] = 0x80
  // Append 64-bit big-endian bit length (high 32 bits assumed 0 for our inputs).
