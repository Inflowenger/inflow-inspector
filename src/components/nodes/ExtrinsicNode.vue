...
const emit = defineEmits<{ (e: 'open-extrinsic-settings'): void }>()
...
const hasSettings = computed(() =>
  !!props.data.serviceTopic ||
  typeof props.data.timeout === 'number' ||
  (props.data.operationData && Object.keys(props.data.operationData).length > 0)
)

function openExtrinsicSettings() {
  emit('open-extrinsic-settings')
}
...
<template>
  <div class="custom-node" :class="{ 'has-meta': hasKey || hasScope, 'has-settings': hasSettings }">
    ...
    <div class="node-actions">
      <!-- Extrinsic settings button -->
      <button
        class="action-btn"
        :class="{ active: hasSettings }"
        title="Open extrinsic settings"
        @click.stop="openExtrinsicSettings"
      >
